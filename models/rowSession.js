...
RowSession.prototype.laps = function () {
  ...
  for (var i = 0; i < totalLaps; i++) {
    ...
    laps.push({
      start: dateFormat(startTime, "isoDateTime"),
      end: dateFormat(endTime, "isoDateTime"),
      meters: lapSize,
      seconds: seconds,
      watt: wattValue,
    });
...
RowSession.prototype.startRow = function (simulate = false) {
  ...
  this.waterrower.datapoints$
    .filter((d) =>
      ["distance", "stroke_average", "speed_average", "clock_down"].some(
        (n) => d.name === n
      )
    )
    .subscribe((d) => {
...
RowSession.prototype.getAvgHr = function () {
  if (this.rawHr.length > 0) {
    return (
      (this.rawHr.reduce(function (total, num) {
        return total + num;
      }) /
        this.rawHr.length) |
      0
